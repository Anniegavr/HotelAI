...
def instantiate(template, values_dict):
    ...
    # elif isinstance(template, basestring):
        ...
        res = AIStringToPyTemplate(template) % values_dict
        return res
...
def match(template, AIStr):
    ...
    try:
        # print(template)
        # print(AIStr)
        regex_template = re.sub('\(\?[a-z]\)', '[a-z]+', template)
        res = re.match( regex_template, AIStr )
        if res != None and res.regs[0][1] == len(AIStr):
            subject = re.findall('^[a-z]+',AIStr)
            res = {'x':subject[0]}
        return res
...
class IF(object):
    ...
    def apply(self, rules, apply_only_one, verbose):
        ...
        # print("Apply()-> new_rules:", new_rules)
        # print("Apply()-> _conditional:", self._conditional)
        # print("Apply()-> _action:", self._action)
        # print("Apply()-> _delete_clause:", self._delete_clause)
        # print("Apply()-> bindings:", bindings)
        for k in bindings:
            for a in self._action:
                fresh_rule = populate(a, k)
                new_rules.add(fresh_rule)
